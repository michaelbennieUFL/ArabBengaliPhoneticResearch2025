...
def generateTestWordList(word_set):
    result_sets = {
        ...
        "allCVCFillers": generateAllCVCFillers(word_set),
        "allVCVFillers": generateVCVFillers(word_set),
        "allSCVCFillers": generateSCVCFillers(word_set),
        "allBisyllabicFillers": generateBisyllabicFillers(word_set),
    }

    # Deduplicate filler word lists
    for filler_key in ["allCVCFillers", "allVCVFillers", "allSCVCFillers", "allBisyllabicFillers"]:
        original_fillers = result_sets[filler_key]["fillers"]
        seen_words = set()
        deduplicated_fillers = []
        for filler in original_fillers:
            word_str = filler if isinstance(filler, str) else filler["word"]
            if word_str not in seen_words:
                seen_words.add(word_str)
                deduplicated_fillers.append(filler)
        result_sets[filler_key]["fillers"] = deduplicated_fillers
